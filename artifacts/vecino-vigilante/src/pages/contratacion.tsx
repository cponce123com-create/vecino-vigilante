...
export default function ContratacionDetalle() {
  ...
  if (error || !data) {
    return (
      <div className="container mx-auto px-4 py-20 text-center">
        <h1 className="font-serif text-3xl font-bold text-destructive mb-4">
          Contratación no encontrada
        </h1>
        <p className="text-muted-foreground">
          La contratación que buscas no existe o ha ocurrido un error.
        </p>
  ...
  // Calcular diferencia entre monto referencial y adjudicado
  ...
  // Extraer datos útiles del rawOcds
  const raw = data.rawOcds as Record<string, string> | null;
  const numeroExpediente = raw?.["Entrega compilada:Licitación:ID de licitación"] || null;
  const fuente = raw?.["Entrega compilada:Licitación:Fuente de financiamiento"] || null;
  const categoria = raw?.["Entrega compilada:Licitación:Categoría principal de contratación"] || null;

  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: data.titulo,
        text: `Contratación pública en Chanchamayo: ${data.titulo}`,
    ...
    <div className="container mx-auto px-4 py-8 space-y-8 max-w-6xl">
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-6 relative overflow-hidden">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="space-y-2">
            <h3 className="font-serif text-xl font-bold text-yellow-900">
              Traducción ciudadana
            </h3>
            <p className="text-yellow-800 leading-relaxed">
              La entidad <strong>{data.entidadNombre}</strong> realizó un proceso de{" "}
              <strong>
                {PROCEDIMIENTO_MAPPING[data.procedimiento || ""]
                  ?.split("(")[0]
                  .trim()
                  .toLowerCase() || data.procedimiento?.toLowerCase()}
              </strong>{" "}
              para contratar{" "}
              <strong>
                {TIPO_MAPPING[data.tipo || ""]?.toLowerCase() || data.tipo?.toLowerCase()}
              </strong>
              .{" "}
              {data.proveedorNombre ? (
                <>
                  El ganador fue <strong>{data.proveedorNombre}</strong> por un monto
                  de <strong>{formatCurrency(data.montoAdjudicado)}</strong>.
                </>
              ) : (
            ...
            {isDirecta && (
              <div className="flex items-center gap-2 text-destructive bg-destructive/10 px-3 py-2 rounded-md w-fit">
                ...
                <span className="font-medium text-sm">
                  Alerta: esta compra se realizó sin proceso competitivo abierto.
                </span>
              </div>
            )}
            ...
            {diferenciaMonto !== null && Math.abs(diferenciaMonto) > 5 && (
              <div
                className={`flex items-center gap-2 px-3 py-2 rounded-md w-fit text-sm font-medium ${
                  diferenciaMonto > 0
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-2 space-y-8">
              ...
              <CardContent className="p-6">
                ...
                {data.plazoEjecucionDias && (
                  <p className="text-sm text-muted-foreground mt-1 flex items-center gap-1">
                    ...
                    Plazo: {data.plazoEjecucionDias} días
                  </p>
                )}
              </CardContent>
            </Card>
          ...
          {/* Información adicional del rawOcds */}
          {(fuente || numeroExpediente || categoria) && (
            <Card>
              <CardHeader>
              ...
              <CardContent className="space-y-3 text-sm">
                {numeroExpediente && (
                  <div className="flex justify-between items-center py-2 border-b">
                    <span className="text-muted-foreground">N° de expediente</span>
                    <span className="font-medium font-mono">{numeroExpediente}</span>
                  </div>
                )}
                {fuente && (
                  <div className="flex justify-between items-center py-2 border-b">
                    <span className="text-muted-foreground">Fuente de financiamiento</span>
                    <span className="font-medium">{fuente}</span>
                  </div>
                )}
                {categoria && (
                  <div className="flex justify-between items-center py-2 border-b">
                    <span className="text-muted-foreground">Categoría OCDS</span>
                ...
                <div className="flex justify-between items-center py-2 border-b">
                  ...
                  <span className="font-medium">
                    {PROCEDIMIENTO_MAPPING[data.procedimiento || ""]?.split("(")[0].trim() || data.procedimiento}
                  </span>
                </div>
                <div className="flex justify-between items-center py-2">
                  <span className="text-muted-foreground">Moneda</span>
                  <span className="font-medium">{data.moneda || "PEN"}</span>
                </div>
              </CardContent>
            </Card>
          )}
                  ...
                  <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded border border-border bg-card shadow-sm">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <time className="text-sm font-medium text-primary">
                        {formatDate(data.fechaConvocatoria)}
                      </time>
                    </div>
                    <p className="text-sm text-muted-foreground">
                      La entidad publicó la necesidad de contratar.
                    </p>
                  </div>
                </div>
                    ...
                    <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded border border-border bg-card shadow-sm">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <time className="text-sm font-medium text-accent">
                          {formatDate(data.fechaAdjudicacion)}
                        </time>
                      </div>
                      <p className="text-sm text-muted-foreground">
                        Se seleccionó al proveedor ganador.
                        {data.proveedorNombre && (
                          <> Ganó: <strong>{data.proveedorNombre}</strong>.</>
                        )}
                      </p>
                    </div>
                  </div>
                ...
                {(data.fechaContrato ||
                  ...
                  <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
                    ...
                    <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded border border-border bg-card shadow-sm">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {data.plazoEjecucionDias
                          ? `Contrato firmado. Plazo de ejecución: ${data.plazoEjecucionDias} días.`
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              <Button className="w-full justify-start" variant="outline" asChild>
                <a
                  href={`mailto:denuncias@osce.gob.pe?subject=Reporte de anomalía: ${data.nomenclatura || data.ocid}&body=Encontré una anomalía en la siguiente contratación:%0A%0A${window.location.href}`}
                >
                  <AlertTriangle className="mr-2 h-4 w-4 text-alerta" />
                  Reportar anomalía al OSCE
                </a>
              </Button>
              ...
              <div className="pt-3 border-t">
                <Button className="w-full justify-start" variant="secondary" asChild>
                  <a
                    href="https://buscadorpublico.seace.gob.pe/"
                    target="_blank"
                    rel="noopener noreferrer"
                  >
                    <ExternalLink className="mr-2 h-4 w-4" />
                    Ver expediente en SEACE
                  </a>
                </Button>
              </div>
            </CardContent>
              ...
              {(data.ubigeoDistrito || data.ubigeoProvincia) && (
                <div className="flex justify-between items-center py-2 border-t">
                  ...
                  <span className="flex items-center gap-1 font-medium">
                    ...
                    {data.ubigeoDistrito}
                    {data.ubigeoProvincia ? `, ${data.ubigeoProvincia}` : ""}
                  </span>
                </div>
              )}
              {data.entidadRuc && (
