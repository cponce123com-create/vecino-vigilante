...
import { eq, and, gte, lte, ilike, sql, desc, asc, isNotNull, or } from "drizzle-orm";
import {
...
function buildContratacionFilters(params: {
  ...
  const conditions = [];
  if (params.ubigeo) {
    conditions.push(eq(contratacionesTable.ubigeoCodigo, params.ubigeo));
  }
  if (params.tipo) {
    conditions.push(eq(contratacionesTable.tipo, params.tipo));
  }
  if (params.estado) {
    conditions.push(eq(contratacionesTable.estado, params.estado));
  }
  if (params.procedimiento) {
    conditions.push(eq(contratacionesTable.procedimiento, params.procedimiento));
  }
  if (params.entidadRuc) {
    conditions.push(eq(contratacionesTable.entidadRuc, params.entidadRuc));
  }
  if (params.proveedorRuc) {
    conditions.push(eq(contratacionesTable.proveedorRuc, params.proveedorRuc));
  }
  if (params.fechaDesde) {
    conditions.push(gte(contratacionesTable.fechaConvocatoria, new Date(params.fechaDesde)));
  }
  if (params.fechaHasta) {
    conditions.push(lte(contratacionesTable.fechaConvocatoria, new Date(params.fechaHasta)));
  }
  if (params.montoMin != null) {
    conditions.push(gte(sql`CAST(${contratacionesTable.montoAdjudicado} AS DECIMAL)`, params.montoMin));
  }
  if (params.montoMax != null) {
    conditions.push(lte(sql`CAST(${contratacionesTable.montoAdjudicado} AS DECIMAL)`, params.montoMax));
  }
  if (params.q) {
    conditions.push(
      or(
...
router.get("/contrataciones", async (req, res): Promise<void> => {
  ...
  const where = buildContratacionFilters({
    ubigeo,
    tipo,
    estado,
    procedimiento,
  ...
  const [rows, countResult, montoResult] = await Promise.all([
    ...
    db
      ...
      .where(where),
    db
      .select({ total: sql<string>`SUM(CAST(${contratacionesTable.montoAdjudicado} AS DECIMAL))` })
      .from(contratacionesTable)
      .where(where),
  ]);
